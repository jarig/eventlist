...
from party.forms import CreatePartyForm, CustomPartyScheduleForm, PartyScheduleFormSet
...
def _credit(request, initialCreatePartyForm=None, initialEventSchedulesFormSet=None, party=None):
    ...
    if request.POST:
        ...
        if createPartyForm.is_valid():
            party = createPartyForm.saveParty(request.user, invited)
...
def invitationList(request, eventScheduleId):
    if request.POST:
        ...
        try:
            party = Party.objects.get(eventSchedule=eventScheduleId, author=request.user)
