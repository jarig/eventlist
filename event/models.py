...
class Event(models.Model):
    ...
    activities = models.ManyToManyField(EventActivity, blank=True, null=True,
                                        related_name='activities')  # event activities/actions
...
def event_group_name(instance, filename):
    return "event_groups/logo/group_logo_%s" % uuid.uuid4()
...
class EventGroup(models.Model):
    ...
    """
    name = models.CharField(max_length=255)
    icon = ImagePreviewModelField(upload_to=event_group_name, max_height=300, max_width=300)
    pass